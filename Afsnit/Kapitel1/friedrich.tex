...
%! Tjek nedenstående ligning, om den giver mening
\begin{equation*}
    \|u\|_{ H^k (\Omega)} \leq C_k |u|_k \quad \forall u \in H_0^k (\Omega).
\end{equation*}

%! Til nedenunder: Vi kigger på C^{\infinity}, på grund af det tidligere nævnte denistets argument
...
\end{equation}
%! Uddyb. Fordi u \in C^{\infinity} => \partial^k \in C^{infinity} og i forlængelse også i H^1_0
Thus $\partial^\alpha u \in H^1_0(\Omega)$. Then by the base case, we have
\begin{equation}
    \begin{split}
        %! Summer skal fjernes og der skal indsættes en ny ulighed
