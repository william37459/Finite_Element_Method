...
To find the solution $u$, we  %CHECK HER: Til linje 100
split $H$, a Hilbert space of functions, 
...
which no longer require $u \in C^2(\OO)$.
\begin{defn}{\quad}
...
\end{defn}
A weak solution only requires $u\in H_0^1(\OO)$, and is therefore less strict. %CHECK HER: Giver denne overgang mening?
There is therefore a higher chance of a solution existing for any given problem, 
...
a similar construction here. So, from \eqref{eq:b_v_problem}, let 
$\tilde{u} \in C^2(\OO)\cap C^0(\bar{\OO}) \cap H^1(\OO)$ be a function such that 
\begin{equation*}
   \tilde{u} = g \quad \text{on } \partial \OO.
\end{equation*}
Let $w = u-\tilde{u}$ and $\tilde{f} = f-L\tilde{u}$. We now have the homogeneous 
Dirichlet problem 
\begin{align}
\begin{split}
...
By Theorem~\ref{thm:existence_hom_dirichlet},
 a weak solution $w \in H_0^1(\OO)$ exists. As $(L\tilde{u},v) = a(\tilde{u},v)$, %Hvilken norm med L?
we get:
\begin{alignat*}{2}
...
as have been several times before, it suffices to look at $\tilde{u}\in H^1(\OO)$.
This is not always possible. %CHECK HER: Skal denne linje med?
%TODO Hvorfor u i H^1? Density assumptions?  #CHECK HER: Pointen med inhomogen er, at den ikke er 0 på kanten, så det er derfor den ikke er inden vi trækker \tilde{u} fra.
